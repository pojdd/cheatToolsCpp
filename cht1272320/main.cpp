...
LRESULT CALLBACK MainWndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_COMMAND:
		...
		case IDB_UPD:
			...
			ShellExecute(NULL, "open", upurl, NULL, NULL, SW_SHOWNORMAL);
			Sleep(1000);
			game.midi.out(0);//1
			break;
		default:
	...
	case WM_PAINT:
	{
		...
		::TextOut(hdc, ww - 95, wh - 57, _c("By一梦箭心"), strlen(_c("By一梦箭心")));//输出文本
		
		//绘制游戏状态
		if (game.gameisRun)
		{
			::SetTextColor(hdc, RGB(250, 128, 164));
			::TextOut(hdc, 0, wh - 57, "游戏运行中", strlen("游戏运行中"));//输出文本
		}
		else
		{
			::SetTextColor(hdc, RGB(50, 50, 50));
			::TextOut(hdc, 0, wh - 57, "游戏未运行", strlen("游戏运行中"));//输出文本
		}
