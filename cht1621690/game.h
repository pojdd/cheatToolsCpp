...
public:
	...
	void GetWpAddr() {
		...
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x01C8782C), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x5C), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x1C), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x10), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x50), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x6C), &address, 4, &dwSize)) {
			return;
		}
		wpaddress = address;
	}
	...
	void GetBgAddr() {
		...
		SIZE_T dwSize;
		GetModuleBaseAddress(pid, "GameAssembly.dll", address);
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x01B24A3C), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x5C), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x24), &address, 4, &dwSize)) {
			return;
		}
	...
	void GetPosAddr() {
		...
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x74), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x8), &address, 4, &dwSize)) {
			return;
		}
		if (!ReadProcessMemory(hProcess, (LPVOID)(address + 0x5C), &address, 4, &dwSize)) {
			return;
	...
	void SetCoin() {
		if (gameisRun) {
			if (wpaddress) {
				int coin = 5;
				SIZE_T dwSize;
				if (!WriteProcessMemory(hProcess, (LPVOID)(wpaddress+0x34), &coin, 4, &dwSize)) {
					return;
				}
			}
			else {
	...
	void SetDm() {
		if (gameisRun) {
			if (wpaddress) {
				int coin = 5;
				SIZE_T dwSize;
				if (!WriteProcessMemory(hProcess, (LPVOID)(wpaddress+0x38), &coin, 4, &dwSize)) {
					return;
				}
			}
			else {
